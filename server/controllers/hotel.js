...
export const createHotelB = async (req, res) => {
    const hotel = req.body;
...
export const updateHotelB = async (req, res) => {
    const { id } = req.params;
    const { title, message, creator, address, price, stars, selectedFile, tags } = req.body;
    
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send(`No hotel with id: ${id}`);

    const updatedHotel = { creator, title, message, tags, address, price, stars, selectedFile, _id: id };
